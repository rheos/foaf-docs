...
FOAF is layered. The default credit graph lives bilaterally on counterparty devices, off-chain; the chain doesn't need to see every neighbourhood trade. The protocol stack (FOAF token, vFOAF and rFOAF staking, RHEO accrual and consumption, governance, treasury) lives on Radix. An optional on-chain settlement path lets specific trustlines move on-chain when they need full proof: high-value trades, escrow, regulatory contexts. Radix is the chain because each of those on-chain layers benefits from primitives that aren't true on most other chains.

At the protocol layer, mutual credit comes down to resources held under conditional authority. vFOAF and rFOAF aren't custom token contracts imitating ERC-20s; they are resources whose lock-up and decay rules live in the engine itself. RHEO doesn't have to materialize as a held balance; it can be minted and burned in the same manifest as the operation that consumes it. Governance authority is itself a resource (a badge), not a permission flag stitched on top of token transfers. The same primitives that make Scrypto pleasant to write make these patterns feel native rather than bolted on.

The optional on-chain settlement path is where Cerberus earns its keep. When a trustline opts into on-chain proof, or when a credit loop settles on chain, an atomic transaction across independent Scrypto components is a single manifest that either commits everywhere or commits nowhere. For trades that need that guarantee, the property is load-bearing rather than ornamental, and not something you can build cleanly on chains that force sequential transactions across components.
...
The team is betting on Radix because the resource model fits what FOAF's on-chain layers actually are. If you've been building in Scrypto and looking for a project where asset-oriented programming earns its keep on something more substantive than another DeFi primitive, this is one.
